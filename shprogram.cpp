...
ShaderProgram::ShaderProgram(const GLchar* vertexPath, const GLchar* fragmentPath)
{
	...
	glAttachShader(shID, vertex_id);
	glAttachShader(shID, fragment_id);
	glLinkProgram(shID);
	...
	{
		GLchar infoLog[512];
		glGetProgramInfoLog(shID, sizeof(infoLog), NULL, infoLog);
		string msg = string("Shader program linking: ") + infoLog;
		throw exception(msg.c_str());
	}

	lightColorLoc = glGetUniformLocation(shID, "lightColor");
	lightPosLoc = glGetUniformLocation(shID, "lightPos");
	viewPosLoc = glGetUniformLocation(shID, "viewPos");
	viewLoc = glGetUniformLocation(shID, "view");
	projectionLoc = glGetUniformLocation(shID, "projection");
	moveLoc = glGetUniformLocation(shID, "move");
	specLoc = glGetUniformLocation(shID, "specularStrength");
